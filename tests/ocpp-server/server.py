...
class ChargePoint(ocpp.v201.ChargePoint):
    ...
    async def route_message(self, raw_msg):
        ...
        asyncio.create_task(self._forward_to_pipeline())
        await super().route_message(raw_msg)

    async def _send(self, message):
        msg_str = str(message) if not isinstance(message, str) else message
        self._message_history.append(msg_str)
        if len(self._message_history) > 10:
            self._message_history.pop(0)
        asyncio.create_task(self._forward_to_pipeline())
        await super()._send(message)

    async def _forward_to_pipeline(self):
        import json
        import urllib.request
        import urllib.error
        
        url = "http://localhost:3000/evaluate"
        data = json.dumps({"sequence": list(self._message_history)}).encode("utf-8")
        ...
        try:
            # Send asynchronously in a thread executor to avoid blocking the server loop
            response = await asyncio.to_thread(urllib.request.urlopen, req, timeout=1.0)
            resp_data = json.loads(response.read().decode("utf-8"))
            if resp_data.get("is_anomaly"):
                logging.warning(f"⚠️ Anomaly Detected! Sequence: {resp_data.get('sequence')} | Avg Prob: {resp_data.get('average_probability')}")
        except Exception:
            # Fail silently if BentoML endpoint is offline
            pass
