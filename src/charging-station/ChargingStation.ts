...
import { existsSync, type FSWatcher, mkdirSync, readFileSync, rmSync, appendFileSync } from 'node:fs'
...
export class ChargingStation extends EventEmitter {
  ...
  public openWSConnection (
    ...
    const originalSend = this.wsConnection.send.bind(this.wsConnection)
    this.wsConnection.send = (message: any, cb?: any) => {
      this.logRawOcppFrame('outgoing', typeof message === 'string' ? message : message.toString())
      originalSend(message, cb)
    }
  ...
  private async onMessage (data: RawData): Promise<void> {
    this.logRawOcppFrame('incoming', data.toString())
  ...
  private logRawOcppFrame (direction: 'incoming' | 'outgoing', message: string): void {
    try {
      const logEntry = JSON.stringify({
        timestamp: new Date().toISOString(),
        stationId: this.stationInfo?.chargingStationId ?? this.hashId,
        direction,
        message,
      })

      const isFault =
        message.includes('INVALID_RFID_CARD') ||
        message.includes('Blocked') ||
        message.includes('Invalid') ||
        message.includes('Faulted') ||
        message.includes('GroundFailure') ||
        message.includes('invalid-tx-uuid-99999') ||
        message.includes('999999')

      const fileName = isFault ? 'ocpp-fault-dataset.jsonl' : 'ocpp-normal-dataset.jsonl'

      // Write to LOGS/
      const logDir1 = join(process.cwd(), 'LOGS')
      if (!existsSync(logDir1)) {
        mkdirSync(logDir1, { recursive: true })
      }
      appendFileSync(join(logDir1, fileName), logEntry + '\n')

      // Write to logs/
      const logDir2 = join(process.cwd(), 'logs')
      if (!existsSync(logDir2)) {
        mkdirSync(logDir2, { recursive: true })
      }
      appendFileSync(join(logDir2, fileName), logEntry + '\n')
    } catch (error) {
      // Ignore write errors to avoid crashing the simulator
    }
  }
}
